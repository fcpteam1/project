		...
		editItem.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e) {
				...
				if(customerTableListener != null) {
					...
					formPanel.removeAll();
					try {
						formPanel.setEditDataAddress(Shop.getInstance().getEditCustomerAddress());
					} catch (IOException e3) {
						// TODO Auto-generated catch block
						e3.printStackTrace();
					}
					try {
						formPanel.setEditDataNumber(Shop.getInstance().getEditCustomerNumber());
					} catch (IOException e2) {
						// TODO Auto-generated catch block
						e2.printStackTrace();
					}
					try {
						formPanel.setEditDataName(Shop.getInstance().getEditCustomerName());
